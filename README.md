...
./gradlew integrationTest  # интеграционные тесты, нужен запущенный Docker
./gradlew check            # все проверки, используется в CI
```

Интеграционные тесты автоматически запускают PostgreSQL 15 через Testcontainers.
